...
class SuperBasicGame(val handler: Handler, val callback: MoveCallback) {
    ...
    private fun playWithDelay(offense: BattleBoard, defense: BattleBoard) {
        handler.postDelayed({
            ...
            playMove(offense, defense)
            callback.onMovePlayed(this)

            if (defense.didUserLose) {
                callback.onGameEnd()
            } else {
                playWithDelay(defense, offense)
            }
    ...
    interface MoveCallback {
        fun onMovePlayed(gmae: SuperBasicGame)
        fun onGameEnd()
    }
}